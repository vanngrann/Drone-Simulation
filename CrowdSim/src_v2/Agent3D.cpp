...
//	}
	...
	//if (truckPos.GetX() == destination3D[jobIndex3D].first && truckPos.GetY() == destination3D[jobIndex3D].second) release = true;
	//else
	//	release = false;
	if (!removeDrone) {
		if (pathCells3D.empty()) {
			if (agDebug3D) cout << "Calling FindPathToRandomFreeCell!" << endl;
			cout << "Truck Position: " << truckPos << endl;
//			Vector3d t(truckPos.GetX(), truckPos.GetY(), truckPos.GetZ());
			Vector3d t(0, 0, 20);
			cout << "t = " << t << endl;
			pos = t;
