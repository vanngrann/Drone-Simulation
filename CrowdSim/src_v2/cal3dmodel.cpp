...
bool Cal3dModel::OnInit(const std::string& strFilename) {
  ...
  for(line = 1; ; line++) {
    ...
    if(strKey == "scale") {
      ...
      // load core skeleton
      if(verboseDebug) std::cout << "Loading skeleton '" << strData << "'..." << std::endl;
      ...
      // load core animation
	  if (verboseDebug) std::cout << "Loading animation '" << strData << "'..." << std::endl;
      ...
      // load core mesh
	  if (verboseDebug) std::cout << "Loading mesh '" << strData << "'..." << std::endl;
      ...
      // load core material
	  if (verboseDebug) std::cout << "Loading material '" << strData << "'..." << std::endl;
...
void Cal3dModel::RenderMesh(bool bWireframe, bool bLight) {
  ...
  //glDisable(GL_CULL_FACE); //->sam change
  ...
  for(meshId = 0; meshId < meshCount; meshId++) {
    ...
    for(submeshId = 0; submeshId < submeshCount; submeshId++) {
      ...
      if(pCalRenderer->selectMeshSubmesh(meshId, submeshId)) {
        ...
        materialColor[3] = 1.0f;//meshColor[3] / 255.0f;
        //cout << " materialColor: [ " << materialColor[0] << "," << materialColor[1] << "," << materialColor[2] << ";" << materialColor[3] << " ]" << endl;
        ...
        if(0) cout << " vertexCount: " << vertexCount << endl;
        ...
        //cout << " RenderMesh: vCount: " << vertexCount << " faceCount: " << faceCount << endl;
        ...
        // draw the submesh
        //cout << "size of CalIndex: " << sizeof(CalIndex) << " faceCount: " << faceCount << endl;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    //cout << "-total face count: " << totalFaceCount << endl;
  }
...
#ifdef CAL3D
  //cout << " calling render in Cal3dModel." << endl;
...
//// DEBUG END
  ...
  /* //SAM,Sam,SOR - I commented this out to avoid using theMenu
...
void Cal3dModel::SetPath( const std::string& strPath ) {
  m_path = strPath;
}
